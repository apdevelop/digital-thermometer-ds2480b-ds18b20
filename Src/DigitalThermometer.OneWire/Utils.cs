...
namespace DigitalThermometer.OneWire
{
    ...
    public static class Utils
    {
        ...
        public static string ByteArrayToHexSpacedString(IEnumerable<byte> bytes)
        {
            return String.Join(" ", bytes.Select(b => b.ToString("X2")));
        }

        internal static string ByteArrayToHexSpacedString(IEnumerable<byte> bytes, int offset, int count)
        {
            return String.Join(" ", bytes.Skip(offset).Take(count).Select(b => b.ToString("X2")));
        }
